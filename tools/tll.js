...
const TLL_COMPILER_DIST = path.join(REPO_ROOT, 'bootstrap', 'dist');
const BOOTSTRAP_CLI = path.join(TLL_COMPILER_DIST, 'cli.js');
const BOOTSTRAP_DIR = REPO_ROOT;
...
function cmdRun(file) {
  ...
  const vmRunnerPath = path.join(BOOTSTRAP_DIR, 'runtime', 'vm_run.tllbc');
  ...
  if (!fs.existsSync(vmRunnerPath)) {
    ...
    try {
      execSync('node --max-old-space-size=4096 "' + BOOTSTRAP_CLI + '" build runtime/vm_run.tll', {
