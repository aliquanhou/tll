...
function copyLibAndCompiler(workDir) {
  ...
  fs.copyFileSync(path.join(BOOTSTRAP_DIR, 'compiler', 'compiler.tll'), path.join(workDir, 'compiler.tll'));
  fs.cpSync(path.join(BOOTSTRAP_DIR, 'compiler'), path.join(workDir, 'lib'), { recursive: true });
}
...
const genCompilerScript = 'tools/gen_compiler_bc.tll';
...
fn main() -> void {
    let result = linkAndCompile("runtime/vm_launcher.tll")
...
log(`  => ${cmp.allPass ? 'ALL PASS 鈥?RUNTIME INDEPENDENCE ACHIEVED' : 'FAILED'}`);
log('');
